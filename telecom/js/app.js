...
function form_submit(e) {
	...
	if (error == 0) {
		...
		if (btn.closest('.contact-us')) {
			const thisSection = document.querySelector('.contact-us');
			thisSection.classList.add('contact-us--successful');
			e.preventDefault();
		}
	} else {
...
let addCloseIcone = document.querySelector('.body-bottom-header__close');
let addCloseIconeBg = document.querySelector('.body-bottom-header__bg');
if (iconMenu != null) {
	let delay = 500;
	...
	iconMenu.addEventListener("click", function (e) {
		if (unlock) {
			...
			addCloseIcone.classList.toggle("_active");
			addCloseIconeBg.classList.toggle("_active");
		}
	});
};
...
function menu_close() {
	...
	addCloseIcone.classList.remove("_active");
	addCloseIconeBg.classList.remove("_active");
}
//additional icone-close
if (addCloseIcone != null) {
	addCloseIcone.addEventListener('click', event => {
		body_lock_remove(400);
		menu_close();
	});
}
const testFunc = () => {
	function newFuncTest(e) {
		if (!e.target.closest('.menu__body') && !e.target.closest('.icon-menu')) {
			body_lock_remove(100);
			menu_close();
			thisDoc.removeEventListener('click', newFuncTest, false);
		}
	}
	if (!iconMenu.classList.contains('_active')) {
		thisDoc.addEventListener('click', newFuncTest, false);
	}
};
//=================
//BodyLock
